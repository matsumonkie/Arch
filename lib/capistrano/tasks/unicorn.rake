# I did this cause the default gem for unicorn with cap3 is not working
# so it's just easier to make it myself rather than try to make it work
      ...
      within current_path do
        if test("[ -e #{fetch(:unicorn_pid)} ]")
          if test("kill -0 #{pid}")
            ...
            sleep 5 # wait for unicorn to be killed before finishing
          else
